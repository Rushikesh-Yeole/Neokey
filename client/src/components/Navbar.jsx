...
import { arghash, symDecrypt, symEncrypt } from "./aesbox";

const Navbar = () => {
  const navigate = useNavigate();
  const { backendUrl, isLoggedIn, setIsLoggedIn, setDemo, prod, setSignup, logout} = useContext(AppContext);  
  ...
  return (
      ...
      {<img onClick={() => navigate('/')} src={assets.beta} alt="Logo" className="hidden md:block w-10 mx-2 items-center cursor-pointer" />}
      ...
      {isLoggedIn==='T' ?
      <button className="z-10 flex justify-center ml-auto items-center px-4 py-1 sm:py-1 mr-4 sm:mr-6 sm:px-5 sm:mb-2 rounded-full border border-slate-300 bg-gray-800 shadow-lg text-cyan-300 group relative cursor-pointer glass-card">  
          <div className="z-10  ml-auto text-cyan-200 cursor-pointer group" >
          {<img src={assets.account} alt="Logo" className="w-6 sm:w-7 cursor-pointer border-[#00f9ff]" />}
        ...
        :(
        <>
        <button onClick={() => demoLogin()} className="mr-2 sm:mr-4 ml-auto mb-2 flex items-center text-sm sm:text-lg bg-cyan-200 border border-[#00f9ff] rounded-full px-3 py-1.5 sm:px-4 shadow-xl text-white hover:bg-cyan-200 hover:text-slate-800 glass-card">
          Demo
        </button>
        <button onClick={() => {setSignup(false); navigate('/login');}} className="mr-2 sm:mr-4 mb-2 flex items-center text-sm sm:text-lg bg-cyan-200 border border-[#00f9ff] rounded-full px-3 py-1.5 sm:px-4 shadow-xl text-white hover:bg-cyan-200 hover:text-slate-800 glass-card">
          Login
        </button>
        </>)
        }
      ...
      {/* Hamburger */}
      <button className="z-10 ml-0.5 sm:ml-0 flex justify-center items-center gap-2 px-4 py-1 sm:py-1.5 sm:px-6 sm:mb-2 mb-0.2 rounded-full border border-slate-300 bg-gray-800 shadow-xl text-cyan-300 group relative cursor-pointer glass-card">  
      {<img src={assets.menu} alt="Logo" className="w-6 sm:w-6 cursor-pointer" />}
