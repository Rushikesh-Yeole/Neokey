...
const BifrostPage = () => {
  ...
  useEffect(() => {
    if (isLoggedIn==='F' && receiverOpened && enteredCode.trim().length === 4){setCode(enteredCode);heimdall('close-bifrost')};
  }, [enteredCode, isLoggedIn, receiverOpened]);

  const heimdall = debounce(async (action) =>{
    try {
      let start = performance.now();
      let { data } = await axios.post(`${backendUrl}/auth/${action}`,{code: enteredCode || code});
      ...
      if(data.success){
        ...
        if(isLoggedIn!=='T'){
          ...
          setTimeout(() => {setIsLoggedIn('T'); navigate('/'); window.location.reload();}, 3000);
        }
      ...
      if(action == 'bifrost'){ await axios.post(`${backendUrl}/admin/engage`, { action:"bifrost"});}
    } catch (error) {
      closePortal();
  ...
  if (isLoggedIn==='T') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-gray-900 to-cyan-900 select-none p-4">
        {<Navbar/>}
        {!started ? (
          ...
          <div className="flex flex-col items-center space-y-4 sm:mt-32">
            <div className="text-center text-cyan-100 font-mono text-lg drop-shadow-sm">Bifrost active.<br></br>But not for long.</div>
  ...
  if (isLoggedIn==='F') {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-gray-900 to-cyan-900 select-none p-4">
        {<Navbar/>}
        {!receiverOpened ? (
          <div className="flex flex-col items-center space-y-6">
            <div className="text-center mb-4">
          ...
          <div className="flex flex-col items-center space-y-6 mt-16">
            ...
            {enteredCode.trim().length===8 && <div className="text-center text-cyan-100 font-mono text-lg drop-shadow-sm">Searching secure channel...</div>}
          </div>
        )}
      </div>
    );
