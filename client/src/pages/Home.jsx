...
const Banner = ({ render, demo, isPwa }) => (
  <div>
            ...
            {!render && (
              <span
                onClick={() => window.showPopup({
                  title: "Using Neokey",
                  body: <>Type/select a service name<br /> → Authorize (Master Password) <br /> → Create password for a service<br /> → Reveal & copy password .<br/> { !demo && <><br/> Tap <Shield className="inline-block align-middle mb-1 text-cyan-400"/> to manually lock the session <br/>(Neokey has timed autolock too) </>} {demo && <><br /> <strong>DEMO</strong> <br />Try Retrieving passwords or Bifrost! <br /><br /> (Reveal for Google, Instagram, X, Netflix...) (Creation disabled for demo.) </>}</>,
                  primaryLabel: "Got it"
                })}
      ...
      <div className="flex flex-col items-center">
        ...
        {!signout && (
          <p className="text-white text-sm sm:text-base font-semibold mb-6 text-center">
            ...
            {demo && <><br/><br/>Demo password is <b>'key'</b></>}
          </p>
        )}
        {signout && (
...
const OpenGate = ({ isLocked, selectedService, isFocused, suggestions, loadC, loadR, keyVal, handleInputChange, setIsFocused, handleKeyDown, handleSuggestionClick, setShowModal, handleAction, copyToClipboard, setIsLoggedIn, demo }) => (
  <div className={`glass-card px-6 p-6 sm:p-10 rounded-2xl shadow-glass w-full max-w-[90%] sm:max-w-sm text-slate-700 text-sm sm:text-base transition-all duration-500 ${isLocked ? 'blur-sm pointer-events-none opacity-50' : ''}`}>
    {!demo && <Shield onClick={()=> {CredStore.wipe(); setIsLoggedIn('N')}} className="text-cyan-200 ml-auto w-6 cursor-pointer"/>}
    ...
    <form className='autocomplete="off" space-y-4'>
      ...
      <div className="flex justify-between gap-4">
        ...
        <button
          id="retrieve"
          className={`w-full py-2.5 rounded-full ${keyVal && "bg-cyan-100/60"} border border-[#00f9ff] ${loadR ? "loading-bar" : ""} text-cyan-200 font-medium`}
          onClick={(e) => {
            e.preventDefault();
...
const ConfirmModal = ({ selectedService, scope, sections, setShowModal, setKey, handleAction, switchScope }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center">
...
const FEATURES = [
  {
    ...
    description: <>No storage. No encryption. Nothing to steal. <br />Neokey forges fresh 8-64 character passwords with upto 256 bits of entropy, in runtime, on client-side. <br /> <br /> Unauthorized access mathematically impossible.</>,
  },
  {
    key: "f2",
    title: "Zero-Trace Architecture",
    description: <>No sensitive data stored. No targets to hit. <br />Privacy by design keeps you invisible to the system with nothing collected, nothing retained.</>,
  },
  {
    key: "f3",
    title: "Dynamic Key Versatility",
    description: <>One tap. Infinite unique passwords for any service. <br />No reuse, no memory, no trace.</>,
  },
  {
    key: "f4",
    title: "End-to-End Fortress",
    description: "Advanced encryption and irreversible hashing protect or destroy sensitive data in transit. 2FA guards the gate.",
  },
  {
    key: "f5",
    title: "Reinforced Access Control",
    description: <>Every sensitive action demands your master key. <br />Sessions hold no power, only you do. <br /> Minimal design. Seamless multi-device access.</>,
  },
];
  ...
  <div className="h-auto min-h-screen text-white flex flex-col items-center justify-center px-6 sm:px-20 lg:px-20">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-7xl auto-rows-fr">
      {FEATURES.map(({ key, title, description }, index) => (
        <div
          key={key}
          className="glass-card animate-pulse-smooth p-4 rounded-xl shadow-2xl shadow-glass sm:hover:scale-105 sm:transform sm:transition sm:duration-300"
        >
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn === 'N') {
      ...
      setRender(false);
      if (isLoggedIn && localStorage.getItem('token') && !CredStore.isUnlocked()) {
        setIsLocked(true);
  ...
  const handleUnlock = async (e) => {
    ...
    // if (demo) {
    // let csalt = CredStore.getCsalt();
    // CredStore.setBsalt(hhash("key", csalt));
    // CredStore.setCred(await arghash(password, csalt));
    // setIsLocked(false);
    // setPassword("");
    // fetchServices();
    // return;
  ...
  const handleAction = async (action) => {
    ...
    try {
      ...
      let currentCsalt = ramCsalt;
      let currentBsalt = ramBsalt;
      let cred = ramCred;
  ...
  return (
    <div className={`min-h-screen sm:px-0 select-none cursor-pointer ${render ? 'bg-black' : 'bg-gradient-to-b from-gray-800 via-cyan-900 to-cyan-950'}`}>
      ...
      <div className="h-screen flex flex-col lg:flex-row items-center justify-center pt-6 sm:pt-16 px-6 sm:px-0">
        <Banner render={render} demo={demo} isPwa={isPwa} />
