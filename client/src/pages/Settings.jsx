...
import { hash } from '../components/aesbox';

const Settings = () => {
  const { backendUrl, userServices, setUserServices, alias } = useContext(AppContext);
  const [activeSubmenu, setActiveSubmenu] = useState('account');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Remove deleted services from userServices immediately
        // setUserServices(prev => prev.filter(service => !servicesToDelete.includes(service)));
        // setServicesToDelete([]);
      
      let cryptpassword = hash(password, localStorage.getItem("email"));
      let {data} = await axios.post(`${backendUrl}/user/delServices`, { cryptpassword, services: servicesToDelete });
      if (data.success) { 
  ...
  const handleDeleteAccount = () => {toast.error("Feature coming soon.", { autoClose: 3000 });};
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-cyan-900 to-cyan-950 p-6">
      <div className="mt-6 max-w-8xl mx-auto">
        ...
        <div className="w-full pt-10 text-slate-500">
          <Footer />
        </div>
    </div>
  );
