...
import { Trash2, Key, Shield, RefreshCcw, User, ArrowLeft, Settings as SettingsIcon, Info, Palette } from 'lucide-react';
import axios from 'axios';
import { AppContext } from '../context/AppContext';
import Footer from '../components/Footer';
import { toast } from 'react-toastify';
import { useNavigate } from 'react-router-dom';
import { encrypt, bhash, arghash } from '../components/aesbox';
...
// --- Sub-Component: Manage Services Panel ---
const ServicesPanel = ({ isMobile }) => {
  const { backendUrl, userServices, setUserServices, demo, publicKey } = useContext(AppContext);
  ...
  const handleSave = async () => {
    ...
    if (demo) return toast.info("Can't alter Demo account settings. Please Sign in", { autoClose: 2000 });

    setLoading(true);
    try {
      let csalt = CredStore.getCsalt();
      let bsalt = CredStore.getBsalt();
      await new Promise(resolve => setTimeout(resolve, 100));
      const payload = {
        services: servicesToDelete,
        cryptcred: encrypt(await arghash(password, csalt), publicKey),
        cryptbsalt: encrypt(bsalt, publicKey)
      };
...
// --- Sub-Component: Account Panel ---
const AccountPanel = ({ isMobile }) => {
  const { backendUrl, publicKey, demo, setIsLoggedIn, setDemo } = useContext(AppContext);
  const { alias, userServices } = useContext(AppContext);
  ...
  const delAccount = async () => {
    ...
    try {
      let csalt = CredStore.getCsalt();
      const cryptcred = encrypt(await arghash(password, csalt), publicKey);
...
const Settings = () => {
  ...
  // Set default view for Desktop
  useEffect(() => { if (!isMobile && !activeSubmenu) setActiveSubmenu('services'); }, [isMobile]);
        ...
        <div className="flex h-[80vh]">
          ...
          <div className="w-80 backdrop-blur-md border-r border-slate-700/50 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {MENU_ITEMS.map((item) => (
                <button key={item.id} onClick={() => setActiveSubmenu(item.id)} className={`w-full flex items-center gap-4 p-4 rounded-xl text-left border transition-all ${activeSubmenu === item.id ? 'bg-gradient-to-r from-cyan-600/20 to-blue-600/20 border-cyan-500/50 shadow-lg' : 'border-transparent hover:bg-slate-800/30'}`}>
