...
export const cook = (base, serv, tm, salt) => {
  let food = (serv.toString().trim().toLowerCase()) + (base.toString().trim()) ;
  let hash = rim((food + tm.toString() + salt.toString() + process.env.SALT));
  ...
  while ((nums.length + alps.length) < hash.length) {
    nums.push(num[i]);
    alps.push(alp[j]);
    i = (i + 1) % nl;
    j = (j + 1) % al;
