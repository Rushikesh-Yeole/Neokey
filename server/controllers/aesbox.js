...
export const symEncrypt = (data, salt="x", keyHex=process.env.SALT) => {
  let iv = crypto.randomBytes(12), key = Buffer.from(bhash(keyHex+salt), "hex");
...
export const symDecrypt = (str, salt="x", keyHex=process.env.SALT) => {
  let [iv, tag, enc] = str.split(".").map(fromB64), dec;
  try {
    const decipher = crypto.createDecipheriv("aes-256-gcm", Buffer.from(bhash(keyHex+salt), "hex"), iv);
